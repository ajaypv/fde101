...
<LessonMeta level="Beginner" minutes={52} verified="16 Aug 2026" kind="Book chapter" />
...
```mermaid
flowchart TD
    A["Collect the source"] --> B["Record version and permissions"]
    B --> C["Parse text, headings, tables, and pages"]
    C --> D["Clean and remove duplicates"]
    D --> E["Create searchable chunks"]
    E --> F["Attach metadata"]
    F --> G["Create embeddings"]
    G --> H["Build keyword and vector indexes"]
    H --> I["Validate, update, and delete"]
```
...
### Step 1: collect and version the source
